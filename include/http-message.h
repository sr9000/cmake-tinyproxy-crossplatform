...
#ifndef TINYPROXY_HTTP_MESSAGE_H
#define TINYPROXY_HTTP_MESSAGE_H

// use the "http_message_t" as a cookie or handle to the structure
typedef struct http_message_s *http_message_t;

// Macro to test if an error occurred with the API. All the HTTP message functions will return 0 if
// no error occurred, or a negative number if there was a problem.
#define IS_HTTP_MSG_ERROR(x) (x < 0)

// initialize the internal structure of the HTTP message
extern http_message_t http_message_create(int response_code, const char *response_string);

// free up an _internal_ resources
extern int http_message_destroy(http_message_t msg);

// Send an HTTP message via the supplied file descriptor. This function will add the "Date" header
// before it's sent.
extern int http_message_send(http_message_t msg, int fd);

// Change the internal state of the HTTP message. Either set the body of the message, update the
// response information, or add a new set of headers.
extern int http_message_set_body(http_message_t msg, const char *body, size_t len);
extern int http_message_set_response(http_message_t msg, int response_code,
                                     const char *response_string);

// Set the headers for this HTTP message.  Each string must be NUL ('\0') terminated, but DO NOT
// include any carriage returns (CR) or line-feeds (LF) since they will be included when the
// http_message is sent.
extern int http_message_add_headers(http_message_t msg, const char **headers,
                                    unsigned int num_headers);

#endif // TINYPROXY_HTTP_MESSAGE_H
