...
int mingw_inet_pton(int af, const char *src, char *dst)
{
    switch (af)
    {
    case AF_INET:
...
int mingw_inet_pton4(const char *src, char *dst)
{
...
int mingw_inet_pton6(const char *src, char *dst)
{
    ...
    while ((ch = tolower(*src++)) != '\0')
    {
        ...
        if (ch == '.' && ((tp + MINGW_NS_INADDRSZ) <= endp) &&
                mingw_inet_pton4(curtok, (char*) tp) > 0)
        {
...
#define proxy_inet_pton mingw_inet_pton6
