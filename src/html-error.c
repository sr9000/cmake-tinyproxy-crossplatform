...
int add_new_errorpage(char *filepath, unsigned int errornum)
{
  ...
  if (hashmap_insert(config.errorpages, errornbuf, filepath, strlen(filepath) + 1) < 0)
    return (-1);
...
static char *get_html_file(unsigned int errornum)
{
  ...
  if (hashmap_return_entry(config.errorpages, result_iter, &key, (void **)&val) < 0)
    return (config.errorpage_undef);
  ...
  while (fgets(inbuf, 4096, infile) != NULL)
  {
    for (p = inbuf; *p; p++)
    {
      ...
      case '}':
        ...
        {
          *p = '\0';
          varval = (const char *)lookup_variable(connptr->error_variables, varstart);
          if (!varval)
            varval = "(unknown)";
          r = write_message(connptr->client_fd, "%s", varval);
          in_variable = 0;
        }
        else
        {
          r = write_message(connptr->client_fd, "%c", *p);
        }
...
int send_http_headers(struct conn_s *connptr, int code, const char *message)
{
  ...
  return (write_message(connptr->client_fd, headers, code, message, PACKAGE, VERSION, add));
}
...
int send_http_error_message(struct conn_s *connptr)
{
  ...
  if (!(infile = fopen(error_file, "r")))
  {
    ...
    return (write_message(connptr->client_fd, fallback_error, connptr->error_number,
                          connptr->error_string, connptr->error_string, detail, PACKAGE, VERSION));
  }
...
int add_error_variable(struct conn_s *connptr, const char *key, const char *val)
{
  ...
  return hashmap_insert(connptr->error_variables, key, val, strlen(val) + 1);
}
 ...
 */
int indicate_http_error(struct conn_s *connptr, int number, const char *message, ...)
{
  va_list ap;
  char *key, *val;
